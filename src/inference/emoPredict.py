...
def predict_emotions(model, tokenizer, text, device="cpu", threshold=0.5):
    ...
    # convert to dict
    emotion_probs = {
        emotion: float(probs[i]) for i, emotion in enumerate(plutchik_emotions)
    }

    # binary predictions using threshold
    emotion_binary = {
        emotion: 1 if prob >= threshold else 0
...
def detect_dyads(emotion_probs, threshold=0.3, method="product"):
    ...
    """
    detected_dyads = {"primary": [], "secondary": [], "tertiary": [], "opposition": []}
...
def analyze_text(
    model, tokenizer, text, device="cpu", emotion_threshold=0.5, dyad_threshold=0.3
):
    """
    ...
    """
    # predict emotions
    emotion_results = predict_emotions(
        model, tokenizer, text, device, emotion_threshold
    )

    # detect dyads
    dyads = detect_dyads(emotion_results["probabilities"], threshold=dyad_threshold)
    dominant_dyads = get_dominant_dyads(
        emotion_results["probabilities"], top_n=3, threshold=dyad_threshold
    )
...
def batch_predict(
    ...
    for i in tqdm(range(0, len(texts), batch_size), desc="processing texts"):
        ...
        for j, text in enumerate(batch_texts):
            ...
            text_probs = {
                emotion: float(probs[j][k])
                for k, emotion in enumerate(plutchik_emotions)
            }

            # get predictions
            predictions = {
                e: 1 if p >= emotion_threshold else 0 for e, p in text_probs.items()
            }
            ...
            dominant_dyads = get_dominant_dyads(
                text_probs, top_n=3, threshold=dyad_threshold
            )
...
def save_results_csv(results, output_path):
    ...
    for result in results:
        row = {
            ...
            **{
                f"{e}_pred": 1 if e in result["predicted_emotions"] else 0
                for e in plutchik_emotions
            },
            # dominant dyads
            "dyad_1": result["dominant_dyads"][0]["name"]
            if len(result["dominant_dyads"]) > 0
            else "",
            "dyad_1_strength": result["dominant_dyads"][0]["strength"]
            if len(result["dominant_dyads"]) > 0
            else 0,
            "dyad_1_type": result["dominant_dyads"][0]["type"]
            if len(result["dominant_dyads"]) > 0
            else "",
            "dyad_2": result["dominant_dyads"][1]["name"]
            if len(result["dominant_dyads"]) > 1
            else "",
            "dyad_2_strength": result["dominant_dyads"][1]["strength"]
            if len(result["dominant_dyads"]) > 1
            else 0,
            "dyad_3": result["dominant_dyads"][2]["name"]
            if len(result["dominant_dyads"]) > 2
            else "",
            "dyad_3_strength": result["dominant_dyads"][2]["strength"]
            if len(result["dominant_dyads"]) > 2
            else 0,
            # counts
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", type=str, default="predictions.csv", help="output csv file"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="use adaptive thresholds (default: True)",
    )
    ...
    if args.text:
        ...
        result = predict_emotions_adaptive(
            model, tokenizer, args.text, device, EMOTION_THRESHOLDS
        )
        ...
        dominant_dyads = get_dominant_dyads(
            result["probabilities"], top_n=3, threshold=DYAD_THRESHOLD
        )

        print("\ndominant dyads:")
        for i, dyad in enumerate(dominant_dyads, 1):
            print(
                f"  {i}. {dyad['name']:20s} [{dyad['type']:10s}] strength: {dyad['strength']:.3f}"
            )
            print(
                f"     {dyad['emotions'][0]} ({dyad['prob1']:.3f}) + {dyad['emotions'][1]} ({dyad['prob2']:.3f})"
            )
    ...
    # batch processing from file
        ...
        print(
            f"  texts with multi-label: {(df_results['num_emotions'] > 1).sum()} ({(df_results['num_emotions'] > 1).sum() / len(df_results) * 100:.1f}%)"
        )
        ...
        print("\nmost common dyads:")
        for dyad, count in sorted(
            dyad_counts.items(), key=lambda x: x[1], reverse=True
        )[:10]:
    ...
    # interactive mode (default)
        ...
        while True:
            try:
                ...
                result = predict_emotions_adaptive(
                    model, tokenizer, text, device, EMOTION_THRESHOLDS
                )
